...
function OnboardingWizard() {
    ...
    useEffect(() => {
        ...
        if (!user.onboardingProfileComplete) {
            ...
            setStep(seen ? 2 : 1);
        } else if (!user.onboardingFaceComplete) {
            setStep(3);
        } else {
            setStep(4);
        }
    ...
    return (
        <section>
            ...
            <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                In the next steps you&apos;ll add your profile, register your face from three angles (for secure
                check-ins), and try a quick attendance-style capture — same flow you&apos;ll use on tasks.
            </p>
            <ul className="text-sm text-slate-400 space-y-2 mb-8 list-disc pl-5">
                ...
                <li>Attendance verification preview</li>
            </ul>
            <button
                type="button"
                onClick={onContinue}
...
function AttendanceIntroStep({
    ...
    const startCamera = useCallback(async () => {
        try {
            ...
            setError('Camera access is required to finish attendance setup.');
        }
    }, [stopCamera, setError]);
    ...
    const snap = () => {
        ...
        canvas.getContext('2d')?.drawImage(video, 0, 0);
        stopCamera();
        setPreview(canvas.toDataURL('image/jpeg', 0.55));
    };

    const submit = async () => {
        if (!preview) {
            setError('Capture your face to complete this step.');
            return;
        }
        setError('');
        setBusy(true);
        try {
            const faceSnapshot = await downscaleJpegDataUrl(preview, 720, 0.5);
            await api.post(
                '/onboarding/complete-attendance-intro',
                { faceSnapshot },
                { timeout: 120_000 },
            );
            await onDone();
    ...
    return (
        <section>
            ...
            <p className="text-slate-400 text-sm mb-4">
                This mirrors how you&apos;ll verify attendance in the field: one quick face capture.
            </p>
            ...
            <div className="rounded-2xl overflow-hidden bg-slate-900 border border-slate-700 aspect-[3/4] relative mb-4">
                {!preview ? (
                    ...
                    <img src={preview} alt="Verification" className="w-full h-full object-cover scale-x-[-1]" />
                )}
                <canvas ref={canvasRef} className="hidden" />
            </div>
                ...
                {!preview ? (
                    ...
                    <>
                        ...
                        <button
                            ...
                            disabled={busy}
                            onClick={submit}
                            className="w-full flex items-center justify-center gap-2 bg-emerald-500 text-black font-bold py-3.5 rounded-xl disabled:opacity-60"
                        >
