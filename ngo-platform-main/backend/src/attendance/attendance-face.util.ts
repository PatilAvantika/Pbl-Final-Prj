...
function toSkinToneScore(r: number, g: number, b: number): boolean {
    const cb = 128 - 0.168736 * r - 0.331264 * g + 0.5 * b;
    const cr = 128 + 0.5 * r - 0.418688 * g - 0.081312 * b;
    return cb >= 77 && cb <= 127 && cr >= 133 && cr <= 173;
}

export function parseAttendanceImage(input: string): Buffer {
...
export function decodeAttendanceFrame(input: string): AttendanceFrame {
...
export function frameToEmbedding(frame: AttendanceFrame): number[] {
    ...
    for (let y = 0; y < outputHeight; y += 1) {
        for (let x = 0; x < outputWidth; x += 1) {
            const sampleX = clamp(
                Math.round(cropX + ((x + 0.5) / outputWidth) * cropWidth),
                0,
                frame.width - 1,
            );
            const sampleY = clamp(
                Math.round(cropY + ((y + 0.5) / outputHeight) * cropHeight),
                0,
                frame.height - 1,
            );
...
export function detectFacePresence(frame: AttendanceFrame): number {
    ...
    for (let y = startY; y < endY; y += 2) {
        for (let x = 0; x < halfWidth; x += 2) {
            ...
            const leftGray = toGray(
                frame.data[leftIdx] ?? 0,
                frame.data[leftIdx + 1] ?? 0,
                frame.data[leftIdx + 2] ?? 0,
            );
            const rightGray = toGray(
                frame.data[rightIdx] ?? 0,
                frame.data[rightIdx + 1] ?? 0,
                frame.data[rightIdx + 2] ?? 0,
            );
...
export function estimateLivenessScore(frames: AttendanceFrame[]): number {
    if (frames.length < 2) {
        return 0;
    }
    ...
    for (let y = 0; y < gridHeight; y += 1) {
        for (let x = 0; x < gridWidth; x += 1) {
            ...
            const leftGray = toGray(
                first.data[leftIdx] ?? 0,
                first.data[leftIdx + 1] ?? 0,
                first.data[leftIdx + 2] ?? 0,
            );
            const rightGray = toGray(
                second.data[rightIdx] ?? 0,
                second.data[rightIdx + 1] ?? 0,
                second.data[rightIdx + 2] ?? 0,
            );
...
export function buildAttendanceFaceTemplate(faceEnrollmentSamples: unknown): number[] | null {
    ...
    for (const sample of faceEnrollmentSamples) {
        const dataUrl = sample && typeof sample === 'object' ? (sample as { dataUrl?: unknown }).dataUrl : undefined;
    ...
    const template = new Array<number>(embeddings[0]!.length).fill(0);
...
export function hashAttendanceImage(frame: AttendanceFrame): string {
