...
import { fetchAiReliability, fetchWorkerReliability } from '@/features/intelligence/intelligence-api';
import { ReliabilityScoreCard } from '@/features/intelligence/components/ReliabilityScoreCard';
...
export default function MobileProfile() {
    ...
    const reliabilityQuery = useQuery({
        queryKey: ['intelligence', 'worker-reliability', user?.id],
        queryFn: () => fetchWorkerReliability(user!.id),
        enabled: Boolean(user?.id),
        staleTime: 60_000,
    });

    const aiReliabilityQuery = useQuery({
        queryKey: ['intelligence', 'ai-reliability', user?.id],
        queryFn: () => fetchAiReliability(user!.id),
        enabled: Boolean(user?.id),
        staleTime: 60_000,
    });
                ...
                {user?.id ? (
                    <div className="space-y-3">
                        <ReliabilityScoreCard
                            data={reliabilityQuery.data}
                            isLoading={reliabilityQuery.isPending}
                            error={reliabilityQuery.isError ? (reliabilityQuery.error as Error) : null}
                            onRetry={() => void reliabilityQuery.refetch()}
                        />
                        <AiInsightCard
                            data={aiReliabilityQuery.data}
                            isLoading={aiReliabilityQuery.isPending}
                            error={aiReliabilityQuery.isError ? (aiReliabilityQuery.error as Error) : null}
                            onRetry={() => void aiReliabilityQuery.refetch()}
                        />
                    </div>
                ) : null}
