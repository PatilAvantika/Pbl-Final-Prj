...
export class TasksController {
    ...
    /**
     * Top 3 replacement / assignment candidates — admin/coordinator only.
     * Volunteers cannot see worker rankings; they only see their own task details.
    ...
    @Get('suggestions/:task_id')
    async taskSuggestions(@Param('task_id') taskId: string, @Request() req: any) {
        const organizationId = requireOrganizationId(req);
        return this.decisionIntelligence.suggestWorkersForTask(taskId, organizationId);
    }
